...
def decrypting_requests_for_server(cipher_and_nonce, main_key):
    ...
    :return: Decrypted data in bytes
    """
    cipher, tag, nonce = cipher_and_nonce.split(global_use_for_client.SEPARATOR)
...
#     parts = cipher_and_nonce.split(global_use_for_client.SEPARATOR)
#     cipher = parts[0]
#     # tag = parts[1]
#     nonce = parts[2]
#
...
def encrypt_msg_for_server(data, main_key):
    ...
    cipher_and_nonce = cipher + global_use_for_client.SEPARATOR + nonce  # combining then to one, in order to send them both in one time.
...
def encrypt_msg_for_client_and_dictionary(data, main_key):
    ...
    cipher_and_nonce_and_tag = b"".join([cipher, global_use_for_client.SEPARATOR, tag, global_use_for_client.SEPARATOR, nonce])  # combining them to send in one transmission
...
def receive_message(conn, key, need="string", wait=None):
    ...
    try:
        ...
        if need == "dictionary":
            ...
            cipher_and_nonce = conn.recv(bytes_length).split(global_use_for_client.SEPARATOR)
